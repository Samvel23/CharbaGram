...
export const getAllFollowings = async (): Promise<IResponse> => {
...
export const handlePostComment = async (id:number, text:string):Promise<IResponse> => {
  const response = await Axios.post('/posts/comment/' + id, {text})
  return response.data;
}

export const handlePostCommentDelete = async (id:number):Promise<IResponse> => {
