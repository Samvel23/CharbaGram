...
import {
  getAccount,
  handleBlock,
  cancelRequest as handleCancelation,
  follow as handleFollow,
  unfollow as handleUnfollow,
} from "../../../helpers/api";
...
export const Account = () => {
        ...
        if (response.status == "following") {
          setUser({
            ...
            followers: user.followers
              ? [...user.followers, response.payload as IUser]
              : [response.payload as IUser],
          });
          ...
          setUser({
            ...
            followers:
              ...
              user.followers.filter(
                (e) => e.id != (response.payload as IUser).id
              ),
          });
        }
      });
  ...
  const blockUser = () => {
    if (user?.id) {
      handleBlock(user.id).then((response) => {
        if (response.payload) {
          setUser(response.payload as IAccount)
        } else {
          setUser({
  ...
  return (
    user && !user.connection.blockedMe ?(
      <div className="container mt-5 mb-5">
        {user.connection.didIBlock ? (
          <div className="row block-row">
            <p className="text-danger">
              You blocked this user{" "}
              {user.connection.blockedMe ? "and this user blocked you" : ""}
            </p>
          </div>
        ) : (
          ""
